...
docker buildx imagetools create --tag $REGISTRY_URL/$DIST_NAME:$VERSION $(for PLATFORM in "${PLATFORMS_ARRAY[@]}"; do ARCH=$(echo $PLATFORM | cut -d'/' -f2); echo "$REGISTRY_URL/$DIST_NAME:$VERSION-$ARCH"; done)
